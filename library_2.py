...
def new_get_synthetic_vec(probability_distribution, a,b,c, threshold, compared_vector, all_ids = ALL_IDS, dic_parent = DIC_PARENT, main_word2vec = MAIN_WORD2VEC, id_labels = ID_LABELS):
	'''
	Args:
        a: number of base predicted label from the top a th highest predcited label
        b: number of candidate of candidate label to choose from
        c: number of extra candidate to choose
...
def nearest_neighbor_with_threshold(probability_distribution, top_k, label_pool, threshold, a, b, c, \
    ...
    new_synthetic_vector = new_get_synthetic_vec(probability_distribution, a,b,c, threshold, compared_vector, all_ids, dic_parent, main_word2vec, id_labels)
...
def accuracy_one_synset(threshold, a, b, c, testing_wnid, probs_result_dir, words_result_dir,\
    ...
    for probs_file in os.listdir(probs_result_dir_synset)[:num_sub_sample]:
        ...
        nns = nearest_neighbor_with_threshold(probability_distribution, top_k_nns, label_pool, threshold, a,b,c, all_ids, dic_parent, main_word2vec, id_labels)
        if nns is None:
            continue
...
def accuracy(threshold, a, b, c, testing_wnids, probs_result_dir, words_result_dir,\
    ...
    for testing_wnid in testing_wnids:
        ...
        try:
            count_correct_set, count_total_set = accuracy_one_synset(threshold, a,b,c, testing_wnid, probs_result_dir, words_result_dir,\
